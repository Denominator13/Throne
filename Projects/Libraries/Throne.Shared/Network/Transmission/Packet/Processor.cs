...
namespace Throne.Framework.Network.Transmission
{
    public abstract unsafe class Processor
    {
        ...
        protected Processor(Byte[] receivedPacket)
        {
            Array = receivedPacket;
        }

        protected Processor(Int32 length)
        {
        ...
        public Processor Seek(Int32 position)
        {
            _operatorPosition = position;
            return this;
        }

        public Processor SeekForward(Int32 position)
        {
            _operatorPosition += position;
            return this;
        }
        ...
        public void Resize(int length)
        {
            ...
            if (Array == null || Array.Length == 0) Array = new byte[length];
            else
                fixed (byte* newptr = newArray)
                fixed (byte* copyptr = Array)
        ...
        {
            get { fixed (byte* ptr = Array) return ptr[offset]; }
            set { fixed (byte* ptr = Array) ptr[offset] = value; }
        }
        ...
        public byte[] ReadArray(int length)
        {
            ...
            System.Array.Copy(Array, PositionAndAdvance(length), array, 0, length);
            return array;
        }
        ...
        {
            return new BigInteger(ReadString(ReadInt()));
        }
        ...
        {
            var _out = new byte[Array.Length];
            fixed (Byte* src = Array, dst = _out)
                NativeMethods.memcpy(dst, src, Array.Length);
            return _out;
        }
        ...
        public static implicit operator Boolean(Processor p)
        {
            return p != null;
        }
    }
