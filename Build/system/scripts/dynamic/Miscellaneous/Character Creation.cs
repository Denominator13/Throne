...
public sealed class CharacterCreation : DynamicScript
{
    ...
    public override ExecutionResult Execute(IEnumerator param)
    {
        ...
        var record = param.Current as CharacterRecord;
        if (!record) return ExecutionResult.Failed;
        ...
        ItemManager.Instance.CreateItemRecord(record, 1100003, 0, 0, 0, Item.Positions.Inventory); // small sash

        new MailWriter(record)
            .SetSender("Scotty/Ocularis")
            .SetHeader("Throne Public Test")
            .WriteLine("Welcome to the public test of Throne!")
            .WriteLine("Some features may not work if your client version is lower than 6013, the recommended version is 6020+.", true)
            .WriteLine("Please post any issues at:")
            .WriteLine("https://github.com/sbradno/Throne/issues", true)
            .WriteLine("Type >help for commands.")
            .AttachItem(/* fireworks*/ItemManager.Instance.CreateItemRecord(3000158))
            .Send();
